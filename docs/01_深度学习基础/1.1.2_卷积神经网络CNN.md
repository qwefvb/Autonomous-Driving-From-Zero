...
对于一个小批量数据 $ \mathcal{B} = \{ x*{1}, \dots, x*{m} \} $ ，BN 的操作如下：

1.  计算均值： $ \mu*{\mathcal{B}} = \frac{1}{m} \sum*{i=1}^{m} x\_{i} $
2.  计算方差： $ \sigma*{\mathcal{B}}^{2} = \frac{1}{m} \sum*{i=1}^{m} (x*{i} - \mu*{\mathcal{B}})^{2} $
3.  规范化： $ \hat{x}_{i} = \frac{x_{i} - \mu*{\mathcal{B}}}{\sqrt{\sigma*{\mathcal{B}}^{2} + \epsilon}} $
4.  缩放与平移： $ y*{i} = \gamma \hat{x}*{i} + \beta $
