...
**拓展链接:** -[CNN基础知识——卷积（Convolution）、填充（Padding）、步长(Stride)](https://zhuanlan.zhihu.com/p/77471866)
...
汇聚层用于降低网络对空间位置的敏感度，并实现特征降维：

- **最大汇聚 (Max Pooling)**：选取窗口内最强信号，增强了模型对光照、位移的鲁棒性。
- **平均汇聚 (Average Pooling)**：保留背景平滑特征。
- **全局平均汇聚 (Global Average Pooling, GAP)**：对整个通道取平均值。它是现代分类网络替代全连接层的标准做法，能显著减少过拟合。
...
1.  计算均值： $\mu_{\mathcal{B}} = \frac{1}{m} \sum_{i=1}^{m} x_{i}$
2.  计算方差： $\sigma_{\mathcal{B}}^{2} = \frac{1}{m} \sum_{i=1}^{m} (x_{i} - \mu_{\mathcal{B}})^{2}$
3.  规范化： $\hat{x}_{i} = \frac{x_{i} - \mu_{\mathcal{B}}}{\sqrt{\sigma_{\mathcal{B}}^{2} + \epsilon}}$
4.  缩放与平移： $y_{i} = \gamma \hat{x}_{i} + \beta$
...
> - [何恺明大神的 CVPR 演讲视频](https://www.youtube.com/watch?v=C6tLw-rPQ24)
